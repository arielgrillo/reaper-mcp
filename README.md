...
| `set_track_folder` | Mutation | `folder_track_index: int`, `last_child_track_index: int` | Make one top-level track a native REAPER folder containing the contiguous top-level tracks immediately after it, through the inclusive final child. Existing folder hierarchy in the target range is rejected. | Verified folder and child track states. | `{"folder_track_index": 2, "last_child_track_index": 4}` |
